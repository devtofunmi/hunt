...
const UserDropdown: React.FC<UserDropdownProps> = ({ dropdownOpen, toggleDropdown, onLogout }) => {
  const { user } = useAuth();
    ...
    <nav className="bg-white/10 backdrop-blur-md border-b border-white/20 py-4 px-6 shadow-md w-full fixed top-0 left-0 z-50">
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        ...
        <ul className="hidden md:flex space-x-8 items-center">
          ...
          {isLoggedIn ? (
            <UserDropdown dropdownOpen={dropdownOpen} toggleDropdown={toggleDropdown} onLogout={logout} />
          ) : (
            <li>
              <Link href="/auth/login" className="bg-gradient-to-r from-[#6E00FF] to-[#0096FF] text-white px-5 py-2 rounded-md hover:opacity-90 transition">
                Login
              </Link>
            </li>
          )}
        ...
        <div className="flex items-center gap-3 md:hidden">
          {isLoggedIn && <UserDropdown dropdownOpen={dropdownOpen} toggleDropdown={toggleDropdown} onLogout={logout} />}
      ...
      {isOpen && (
        <div className="md:hidden mt-4 space-y-4">
          ...
          {!isLoggedIn && (
            <Link href="/auth/login" className="block py-2 bg-gradient-to-r from-[#6E00FF] to-[#0096FF] text-center rounded-md text-white hover:opacity-90 transition" onClick={() => setIsOpen(false)}>
              Login
            </Link>
          )}
        </div>
